#' Calculate the distance between each stop for each route/trip
#'
#' @param gtfs A GTFS object from gtfstools
...
calculate_dist_stops <- function(shape_id1) {
...
trechos <- parallel::mclapply(unique(shapes_linhas$shape_id),
                              FUN = purrr::possibly(calculate_dist_stops, otherwise = NA_real_),
                              mc.cores = 5)
